...
mount_height = 8;
mount_width = 20;
mount_length = first_hole + second_hole + M3_nut_D;
...
module nozzle_mount_cuts()
{
  ...
  translate([0,-mount_width/2 + 7/2,4.4]) cube([50,mount_width,mount_height],center=true);
  ...
  translate([-first_hole,0,-mount_height/2 + 4.4]) cylinder(r=7/2,h=mount_height,$fn=30); translate([-first_hole - mount_width,-7/2,-mount_height/2 + 4.4]) cube([mount_width,7,mount_height]);
	
  translate([second_hole,0,-mount_height/2 + 4.4]) cylinder(r=7/2,h=mount_height,$fn=30);
  
  translate([second_hole + mount_width/2,0,0]) cube([mount_width,2*1.6,mount_height+1],center=true);
  translate([second_hole,-7/2,-mount_height/2 + 4.4]) cube([mount_width,7,mount_height]);
}
